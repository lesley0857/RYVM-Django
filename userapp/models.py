...
class Usersocities(models.Model):
    user = models.ForeignKey(
        Custombaseuser, on_delete=models.SET_NULL, null=True, blank=False)
    society = models.ForeignKey(
        Societies, on_delete=models.SET_NULL, null=True, blank=False)
    ...
    def save(self):
        try:
            user = Usersocities.objects.filter(
                user__id=self.user.id, society__id=self.society.id)
            if user.count() >= 1:
                print('more than one')
