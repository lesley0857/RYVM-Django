...
class Custombaseuser(AbstractBaseUser,PermissionsMixin):
    ...
    marital_status = models.CharField(max_length=200,choices=STATUS,default= 'STATUS[0]',blank=False,null=True)
...
class Usersocities(models.Model):
    #POSITION = (('Member','Member'),('Admin','Admin'),('Asst_admin','Asst_admin'),('Chaplain','Chaplain'))
    user = models.ForeignKey(Custombaseuser,on_delete=models.SET_NULL,null=True,blank=False)
    society = models.ForeignKey(Societies,on_delete=models.SET_NULL,null=True, blank=False)
    #position = models.CharField(max_length=200,choices=POSITION,default= POSITION[0],blank=False,null=True)
    ...
    def save(self):
        try:
            user= Usersocities.objects.filter(user__id=self.user.id,society__id=self.society.id)
            if user.count() >= 1:
                print('more than one')
                
            else:
                super(Usersocities,self).save()
        except:
            super(Usersocities,self).save()
